...
The GC-content of the reads in a sample is calculated by obtaining the GC-content of every read in the sample (`seqkit`), and applying the `density()` function in `R` to obtain the density values for 100 bins (from 1 to 100 °C, 1°C intervals). The raw GC content data used for the plots can be found in the ` `r basename(resultsdir)` ` folder.
