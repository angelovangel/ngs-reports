...
### `fastp` modules
[`fastp`]() is a program similar to [`FastQC`](), but much faster. Here, `fastp` is run on the supplied fastq files and various statistics are generated. If quality and adapter trimming was selected in the startup window, it is performed here and the trimmed reads are saved in the results folder under `trimmed-reads`. 


### GC-content of reads
Hover over the read names in the legend to see the trace. Traces are colored according to the peak GC-content of the sample. The raw GC content data used for the plots can be found in the ` `r basename(resultsdir)` ` folder.
