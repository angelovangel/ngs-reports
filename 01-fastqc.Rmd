---
title: "Sample and sequencing QC report"
...
### Number of reads and average read quality
...
Number of fastq files: **`r length(fastqfiles)`** 
...
### `fastp` filtering
All fastq reads are processed with `fastp`, the individual report files can be found in the ` `r paste(basename(resultsdir), "/fastp_reports", sep ="")` ` directory. The values in the table below are per sample, e.g. for both R1 and R2 in case of PE data.
...
fastp_filter_stats %>%
  ...
  #formatStyle('num_seqs', background = styleColorBar(data = c(0, max(read_qual$num_seqs)), "lightgreen"))
...
### Per cycle quality and base content
...
readr::write_delim(gcdf, path = file.path(resultsdir, "gc_content_stats.tsv"))
...
packages <- c("dplyr", "purrr", "tidyr", "readr", "stringr", "apexcharter", "DT", "parallel", "qckitfastq", "kableExtra", "readxl", "data.table")
