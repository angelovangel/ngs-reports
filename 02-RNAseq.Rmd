...
source("R/process_samplesheet.R")
source("R/seqkit_stats.R")
source("R/gc_stats.R")
source("R/summarize_featureCounts.R")
...
header_table <- data.frame(Parameter = c("Project",
                           ...
                           Value = c(params$project,
                                     ...
                                     params$fastq_dir,
                                     #basename(params$fasta_file),
                                     #basename(params$GTF_file),
                                     params$library_prep,
                                     params$sequencer,
                                     params$seqtype,
                                     params$min_map_qual))
...
# if PE sequencing was selected, get for and rev; otherwise treat all fastq files as SE
  ...
  if (!is.null(params$fastq_dir)) {
    ...
    # first check if there are bams already
      if (length(bamfiles) > 0) {
        ...
        #if performing align() for the first time, assign bamfiles again and save rds
...
system2(command = "R/prokka_gff2saf.sh", 
...
source("R/rseqc_read_duplication.R")
...
source("R/rseqc_read_strandness.R")
  ...
  system2(command = "R/prokka_gff2gtf.sh", 
          args = c("-e", "-i", params$GFF_file), 
          stdout = gtffile
  )
  
  # this is a perl script, written by 
  system2(command = "R/gtf2bed", 
          args = c(gtffile), 
          stdout = bedfile
  )
...
This [module](http://rseqc.sourceforge.net/#genebody-coverage-py) calculates the RNAseq reads coverage along the genes. For "normal" RNAseq it is expected that the coverage is uniform, e.g. there is no 5' or 3' bias, and decreases towards the 5' and 3' ends. Note that, for bacterial genomes, the gene boundaries are often not well defined (usually the coding sequences are predicted, not the transcripts) and the coding sequences are regarded as a "genes". As a consequence, the genebody coverage plot may not look as good as for model organisms with well annotated genomes. 

```{r genebody coverage}
# The module requires sorted and indexed bam files, so first this has to be done:
source("R/samtools_si.R")

parallel::mclapply(bamfiles, samtools_sort)

bamfiles_sorted <- list.files(fastqdir, pattern = "sorted.bam$", full.names = TRUE)
parallel::mclapply(bamfiles_sorted, samtools_index)

# after this, just run geneBody_coverage.py on the sorted (and indexed) bams
# this is also slow, so use parallel

# define output folder
genebody_outdir <- file.path("02-RNAseq-results/", "genebody_coverage")
...
genecoverage <- function(bamfile, bedfile) {
  system2(command = "geneBody_coverage.py", 
          args = c("-i", bamfile, "-r", bedfile, "-o", basename(bamfile), sep = "")
          )
}

parallel::mclapply(bamfiles_sorted, genecoverage, bedfile)

# have to move the files manualy here
genebody_outfiles <- list.files(".", pattern = "geneBodyCoverage")
system2("mv", args = c(genebody_outfiles, genebody_outdir))

# -----------------------------------------------
# parsing output og geneBody_coverage.py in R
# use the txt files to get the values 
genebody_outfiles_txt <- list.files(path = genebody_outdir, pattern = "geneBodyCoverage.txt$", full.names = TRUE)

# read one geneBodyCoverage.txt file and return a df
read_genebody <- function(x) {
tibble(filename = basename(x) %>% str_extract(pattern = "^\\w+"),
       counts = fread(x, skip = 1, nrows = 1, drop = 1) %>% t() %>% as.numeric(), 
       percentile_rank = percent_rank(counts),
       percentile = 1:100) 
}
...
genebody_df %>% 
  apex(mapping = aes(percentile, percentile_rank, group = filename), type = "line") %>%
