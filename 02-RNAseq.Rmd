...
bamfiles <- list.files(resultsdir, pattern = "subread.BAM$", full.names = TRUE)

# character vector with output file names, redirect output to resultsdir/bamfiles
align_outfiles <- file.path(resultsdir, paste(tools::file_path_sans_ext(basename(for_files)), ".subread.BAM", sep = ""))
  ...
  if (!is.null(params$fastq_dir)) {
    ...
    # first check if there are bams already
      if (length(bamfiles) > 0) {
        ...
        bamfiles <- list.files(resultsdir, pattern = "BAM$", full.names = TRUE)
